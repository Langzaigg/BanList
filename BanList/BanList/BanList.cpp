...
EVE_PrivateMsg_EX(__eventPrivateMsg)
{
	...
	if (eve.fromQQ != MASTER) return;
		...
		if (setGroupLeave(llgroup) == 0)
			...
			AddMsgToQueue("退出失败或不在此群。", eve.fromQQ);
		eve.message_block();
		return;
	}
	...
	if (eve.fromQQ != MASTER) return;
		...
		AddMsgToQueue("已将" + getStrangerInfo(llqqnum).nick + "(" + strqqnum+ ")列入封禁名单！", eve.fromQQ);
		eve.message_block();
		return;
	}
	else if (strLowerMessage.substr(intMsgCnt, 5) == "unban")
	{
		...
		AddMsgToQueue("已将" + getStrangerInfo(llqqnum).nick + "(" + strqqnum + ")移出封禁名单！", eve.fromQQ);
		eve.message_block();
		return;
	}
	else if (strLowerMessage.substr(intMsgCnt, 4) == "info")
	{
		...
		while (isspace(strLowerMessage[intMsgCnt]))
			intMsgCnt++;
		AddMsgToQueue(getStrangerInfo(eve.fromQQ).nick + "(" + to_string(eve.fromQQ) + "):"+eve.message.substr(intMsgCnt), MASTER);
		eve.message_block();
		return;
	}
	return;
...
EVE_System_GroupMemberDecrease(__eventSystem_GroupMemberDecrease)
{
	if (beingOperateQQ == getLoginQQ())
	{
		...
		AddMsgToQueue("已将" + getStrangerInfo(fromQQ).nick + "(" + to_string(fromQQ) + ")列入封禁名单！" + "原因：被踢出群" + to_string(fromGroup), MASTER);
		return 1;
	}
	return 0;
}
...
EVE_Request_AddGroup(__eventRequest_AddGroup)
{
	if (subType == 2)
	{
		...
		AddMsgToQueue("已通过"+getStrangerInfo(fromQQ).nick + "(" + to_string(fromQQ) + ")的入群邀请。", MASTER);
